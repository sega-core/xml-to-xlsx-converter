...
function createSingleSheet(data) {
  ...
  data.products.forEach((product) => {
    ...
    product.details.forEach((detail) => {
      // Считаем суммы по операциям для детали
      let sumTotalTime = 0;  // E - Сумма строк '№ операции' по графе 'H'
      let sumCost = 0;       // F - Сумма строк '№ операции' по графе 'I'
      ...
      rows.push([
        ...
        detail.quantity * sumTotalTime,                // G - Сумма времени на все детали на изделие (D * E)
        detail.quantity * sumCost                      // H - Сумма в руб на все детали в изделии (D * F)
      ]);
